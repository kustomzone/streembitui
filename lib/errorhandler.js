...
(function () {
    ...
    function handleFindMessages(err) {
        if (!err) {
            return doorclient.notify.error("Error in finding messages: unknow error", null, true);
        }

        if ((err.message && typeof err.message == "string" && err.message.indexOf(NOMSGERR) > -1) ||
            (typeof err == "string" && err.indexOf(NOMSGERR) > -1)) {
            doorclient.notify.info("There are no messages for the account", null, true);
        }
        else {
            doorclient.notify.error("Error in finding messages:  %j", err, null, true);
        } 
    }
    ...
    function handleWebrtcError(err, param) {
        ...
        doorclient.notify.error(msg, null, true);
    }
