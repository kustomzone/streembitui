...
(function () {
    ...
    apputils.clear_database = function () {
        var process = confirm("All settings and data will be removed from the local database. We strongly suggest to clear the database if you use DoorClient on a shared/public computer.  Make sure you made a backup if you want to keep using your account.");
        if (process) {
            database.clear().then(
                function () {
                    doorclient.notify.success("The database was cleared successfully");
                },
                function (err) {
                    doorclient.notify.error("Database clear error %j", err);
                }
            );
        }
    };
    ...
    apputils.backup_account = function() {
        try {
            ...
            filedialog.initialize({
                type: 'save',
                accept: ['doorclient.dat'],
                path: '~/Documents',
                defaultSavePath: 'doorclient.dat'
            });
            ...
            var file_name = "doorclient_" + username + ".dat";
            filedialog.saveTextFile(text, file_name, function () {
                doorclient.notify.success("Account backup is completed.");
            });            

        }
        catch (err) {
            doorclient.notify.error("Account backup error: %j", err);
        }       
    }
    ...
    apputils.backup_contacts = function () {
        try {
            ...
            database.ContactsDB.get_contacts(username, function (err, result) {
                if (err) {
                    return doorclient.notify.error("Database get contacts error %j", err, true);
                }
                ...
                filedialog.saveTextFile(text, file_name, function () {
                    doorclient.notify.success("Contacts backup is completed.", null, true);
                });
            });            
    ...
    apputils.get_pending_contacts = function (callback) {
        database.getall(database.TEMPCONTDB, function (err, result) {
            if (err) {
                ...
                return doorclient.notify.error("Database get pending contacts error %j", err, true);
            }
    ...
    apputils.get_pending_contact = function (account) {
        ...
        try {
            ...
            doorclient.notify.error("Get pending contact error: %j", e, true);
        }
    }
    ...
    apputils.add_blocked_contact = function (contact_name, callback) {
        ...
        database.update(database.BLOCKCONTDB, blocked_contacts).then(
            ...
            function (err) {
                doorclient.notify.error("Update blocked contacts error %j", err, true);
                callback(err);
            }
        );
    }
    ...
    apputils.accept_call = function (sender, type, resultfn) {
        ...
        if (type != defs.CALLTYPE_VIDEO && type != defs.CALLTYPE_AUDIO) {
            return doorclient.notify.error("Invalid call type received from " + sender, null, true);
        }
    ...
    apputils.restore_account = function (callback) {
        try {
            ...
            filedialog.readTextFile(function (err, buffer, path) {
                ...
                try {
                    if (!buffer) {
                        throw new Error("not a valid DoorClient account backup");
                    }
                    ...
                    return doorclient.notify.error("Invalid account backup data. Error: %j", e);
                }

                if (!text) {
                    return doorclient.notify.error("Invalid account backup data");
                }
            ...
            doorclient.notify.error("Account restore error: %j", e);
        }
    }
    ...
    apputils.restore_contacts = function (callback) {
        try {
            ...
            filedialog.readTextFile(function (err, buffer, path) {
                ...
                try {
                    if (!buffer) {
                        throw new Error("not a valid DoorClient contact backup");
                    }
                    ...
                    return doorclient.notify.error("DoorClient contacts restore error: %j", e);
                }
                ...
                bootbox.confirm("Restore the contacts list. All existing contact data will be overwritten", function (result) {
                    if (result) {
                        ...
                        database.ContactsDB.update(username, data, function (err) {
                            if (err) {
                                return doorclient.notify.error("Contacts restore error: %j", e);
                            }

                            doorclient.notify.success("The contacts have been restored.");
                        });
                    }
                });
            });              
            ...
            doorclient.notify.error("Contacts restore error: %j", e);
        }
    }
    ...
    apputils.getversion = function (callback) {
        ...
        $.getJSON('https://api.github.com/repos/authenticity/doorclient/releases', {}, function (data) {
            var version;
