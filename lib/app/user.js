...
(function () {
    ...
    function addToDB(account, publickey, cipher_context, callback) {
        ...
        accounts.update(user, function (err) {
            if (err) {
                return doorclient.notify.error("Database update error %j", err);
            }
    ...
    usrobj.create_account = function (account, password, callback) {
        try {
            ...
            addToDB(account, publicKey, cipher_context, function () {
                ...
                doorclient.ui.set_account_title(account);
    ...
    usrobj.initialize = function (user, password) {
        try {
            if (!user || !password) {
                doorclient.notify.error("Invalid parameters, the account and passwords are required");
                return false;
            }

            var account_name = user.account;
            if (!account_name) {
                doorclient.notify.error("Invalid account name");
                return false;
            }
            ...
            try {
                ...
                if (err.message && err.message.indexOf("decrypt") > -1) {
                    doorclient.notify.error("Account initialize error, most likely an incorrect password was entered");
                }
                else {
                    doorclient.notify.error("Account initialize error: %j", err);
                }
                return false;
            }
            ...
            try {
                ...
                doorclient.notify.error("Account initialize error. Select a saved account and enter the valid password. The encrypted account information must exists on the computer.");
                return false;
            }
            ...
            if (key.publicKeyHex != user.public_key) {
                doorclient.notify.error("Error in initializing the account, most likely an incorrect password");
                return false;
            }
            ...
            doorclient.ui.set_account_title(account_name);

            // the account exists and the encrypted entropy is correct!
            return true;
        }
        catch (err) {
            doorclient.notify.error("Account initialize error. Select a saved account and enter the valid password.");
            return false;
        }
    };
    ...
    usrobj.restore = function (password, user, callback) {
        try {
            ...
            key.keyFromPrivate(hexPrivatekey, 'hex');
            if (key.publicKeyHex != user.public_key) {
                doorclient.notify.error("Error in restoring the account, incorrect password or invalid backup data");
                return false;
            }
            
            if (!userobj.connsymmkey) {
                doorclient.notify.error("Error in restoring the account, incorrect connection key in backup data");
                return false;
            }
            ...
            addToDB(account, publicKey, cipher_context, function () {
                ...
                doorclient.ui.set_account_title(account);
                doorclient.notify.success("The account has been restored and initialized. You can connect to the City of Osmio P2P network");
    ...
    usrobj.change_password = function (user, password, callback) {
        try {
            ...
            if (usrobj.name != account) {
                doorclient.notify.error("Account " + account + " is not initialized");
                return false;
            }
            ...
            if (!appsrvc.privateKeyHex || !appsrvc.connsymmkey) {
                doorclient.notify.error("the existing crypto key is invalid");
                return false;
            }
