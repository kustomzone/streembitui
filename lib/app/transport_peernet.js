...
function TransportPeerNet() {
    ...
    this.create_netclient = function (completefn) {
        var self = this;
        try {
            logger.debug("Initializing peer DoorClient client");
                ...
                function (seeds, callback) {
                    try {
                        if (!seeds || !Array.isArray(seeds) || !seeds.length) {
                            return callback("Error in populating the seed list. Please make sure the 'bootseeds' configuration is correct and a firewall doesn't block the DoorClient software!");
                        }
    ...
    this.init = function (completefn) {
        var self = this;
        try {
            logger.debug("Initializing peer DoorClient");
    ...
    this.setUpnpPort = function (callback) {
        try {
            ...
            client.portMapping(
                {
                    ...
                    description: "DoorClient" 
                },
                function (err, result) {
                    if (err) {
                        doorclient.notify.error("UPNP portMapping error: %j", err.message ? err.message : err, true);
                    }
            ...
            doorclient.notify.error("UPNP portMapping exception: %j", e, true);
            callback(e);
        }
    }    
    ...
    this.onPeerMessage = function(message, info) {
        try {
            if (!message) {
                return doorclient.notify.error("Invalid message at onPeerMessage", null, true);
            }
            if (!message.type || message.type != "PEERMSG") {
                return doorclient.notify.error("Invalid message type at onPeerMessage", null, true);
            }
            if (!message.data) {
                return doorclient.notify.error("Invalid message data at onPeerMessage", null, true);
            }
