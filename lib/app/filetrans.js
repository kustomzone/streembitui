...
(function () {
    ...
    function fileItem(fileobj, filename, filesize, filehash, filetype, filepath, chunksize) {
        ...
        obj.complete_receive = function () {
            if (obj.offset >= obj.size ) {
                if (doorclient.globals.nwmode1) { // TODO the corect is nwmode, nwmode1 is for test
                    //TODO
                }
                else {
                    ...
                    //  verify the hash only for node-webkit instances
                    //  for browser version the hash check produces random errors
    ...
    fproc.cancel_by_peer = function (payload) {
        ...
        doorclient.Session.tasksvm.cancel_by_peer(hash);
    ...
    fproc.initreceive = function (options, oncomplete) {
        try {
            ...
            doorclient.notify.error("File receive error %j", err);
        }
    }
