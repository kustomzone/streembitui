...
(function () {
    ...
    function validate_contact(account, obj) {
        if (!account || !obj) {
            return doorclient.notify.error("validate_contact error: invalid parameters");
        }
    ...
    function init_contact(contact, callback) {
        try {
            ...
            contactobj.find(function(err, result){
                if(err) {
                    var msg = doorclient.notify.error("Couldn't find contact " + account + ", %j", err, true);
                ...
                if (!result || account != result.name) {
                    ...
                    doorclient.notify.error("Couldn't find contact " + account + " on the network", null, true);
                    return callback();
                }
            ...
            callback();
            doorclient.notify.error("init_contact error: %j", err, true);
        }
    }
    ...
    handler.add_contact = function (contact, cbfunc) {
        ...
        function (err) {
            if (err && !isoffer_complete) {
                ...
                doorclient.notify.info("Contact offer was added, but offer from the contact was not received", null, true);
            }
    ...
    handler.init = function (callback) {
        try {
            ...
            async.eachSeries(contacts, init_contact, function (err) {
                ...
                doorclient.notify.info(msg, null, true);

                //
            });
        }
        catch (err) {
            doorclient.notify.error("Error in initializing contacts: %j", err, true);
        }
    }
