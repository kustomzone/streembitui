...
(function () {
    ...
    define(
        ['knockout', 'appsrvc', 'contactlist', 'peercomm', 'appevents', 'apputils', 'doorclientnet', 'settings', 'contactsutil', 'uihandler', 'utilities', './contactsbar.html!text'],
        function (ko, appsrvc, contactlist, peercomm, appevents, apputils, doorclientnet, settings, contactsutil, uihandler, utilities, template) {
            ...
            var viewModel = {
                ...
                onContactError: function (account, error) {
                    try {
                        ...
                        doorclient.notify.error("contact onContactError error %j", err, true);
                    }
                },
                ...
                onLastPing: function (account) {
                    try {
                        ...
                        doorclient.notify.error("contact onLastPing error %j", err, true);
                    }
                },
                ...
                onContactOnline: function (account, isonline) {
                    try {
                        ...
                        doorclient.notify.error("contact onContactOnline error %j", err, true);
                    }
                },
                ...
                onTextMessage: function (data) {
                    try {
                        ...
                        doorclient.notify.error("contact onTextMessage error %j", err, true);
                    }
                },
                ...
                onFileReceive: function (sender, blob, filename, filesize) {
                    try {
                        ...
                        doorclient.notify.error("contact onFileReceive error %j", err, true);
                    }
                },
                ...
                itemAction: function (item) {
                    try {
                        if (item.actiontype == "textmsg") {
                            ...
                            item.actiontype = "";
                            item.actionicon("");
                            doorclient.notify.error("Contact error: %j", item.error());
                            item.error("")
                        }
                    }
                    catch (err) {
                        doorclient.notify.error("contact itemAction error %j", err, true);
                    }
                },
                ...
                search: function () {
                    try {
                        ...
                        if (!account) {
                            return doorclient.notify.error("Enter the human or device account name");
                        }

                        if (contactlist.exists(account)) {
                            return doorclient.notify.info("This contact exists in the contact list");;
                        }

                        doorclientnet.find_contact(account, function (err, contact) {
                            if (err) {
                                return doorclient.notify.error('Error in searching for contact "' + account + '", %j', err);
                            }

                            if (!contact || !contact.account) {
                                return doorclient.notify.info("Couldn't find contact " + account, null, true);
                            }                            
                        ...
                        doorclient.notify.error("contact search error %j", err, true)
                    }
                },
                ...
                onSendAddContactRequest: function () {
                    ...
                    try {
                        ...
                        if (this.addcontact_obj.account == appsrvc.username) {
                            return doorclient.notify.error("Can't add yourself as a contact.");
                        }
                        ...
                        contactsutil.add_contact(contact, function (err) {
                            ...
                            if (err) {
                                return doorclient.notify.error("Add contact error %j", err);
                            }
                        ...
                        uihandler.unblockwin();
                        unblocked = true;
                        doorclient.notify.error("Add contact error %j", err);
                    }
                ...
                acceptAddContact: function (obj) {
                    try {
                        ...
                        doorclient.notify.error("Accept contact error %j", err, true)
                    }
                },
                ...
                declineAddContact: function (obj) {
                    try {                        
                        ...
                        doorclient.notify.error("Decline contact error %j", err);
                    }                    
                },
