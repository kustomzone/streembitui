    ...
    define([
        ...
        function AcceptContRequestVm(params) {
            if (!appsrvc.account_connected) {
                ...
                return doorclient.notify.error("First connect to the DoorClient network");
            }
                ...
                parse: function (value) {
                    try {
                        ...
                        if (!pos < 0) {
                            ...
                            return doorclient.notify.error("Invalid contact request.");
                        }
                        ...
                        if (!decodedmsg) {
                            return doorclient.notify.error("Signature verification failed. Invalid contact offer object.");
                        }
                     
                        var obj = JSON.parse(decodedmsg.data);
                        if (!obj.account || !obj.public_key || !obj.usertype || !obj.port || !obj.address || !obj.transport ) {
                            vm.showcontent(false);
                            return doorclient.notify.error("Invalid contact offer object.");
                        }

                        if (payload.iss != obj.public_key) {
                            vm.showcontent(false);
                            return doorclient.notify.error("Invalid contact offer object. Public key mismatch.");
                        }
                        ...
                        vm.showcontent(false);
                        doorclient.notify.error("Error in parsing the contact offer object.");
                    }
                }
            };
