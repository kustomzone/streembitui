        ...
        var handler = {
            ...
            onerror: function (hash, err, param) {
                try {
                    ...
                    doorclient.notify.error("Send  file VM onerror: %j", err, true)
                }
            },
                ...
                .then(
                    ...
                    function (isaccepted) {
                        ...
                        if (isaccepted == true) {
                            ...
                            doorclient.notify.error("The contact refused to accept the file");
                            appevents.dispatch("on-task-event", "close", "send", params.md5hash);
                        }
                    }
                )
                .catch(function (err) {
                    doorclient.notify.error("Error in starting file transfer: %j", err);
                    appevents.dispatch("on-task-event", "close", "send", params.md5hash);
                });
            },
            ...
            initsend: function (params) {
                try {
                    if (!params || !params.file || !params.md5hash || !params.file.size || !params.contact) {
                        return doorclient.notify.error("Invalid send file parameters", null, true)
                    }

                    if (doorclient.api.taskexists(params.md5hash, params.contact.name)){
                    ...
                    handler.offer(params, function () {
                        try {
                            ...
                            doorclient.notify.error("Error in sending the file: " + e.message, null, true)
                        }
                    });
                }
                catch (e) {
