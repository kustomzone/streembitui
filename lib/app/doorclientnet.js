...
(function () {
    ...
    function netfactory(transport) {
        if ((doorclient && doorclient.globals && doorclient.globals.nwmode) && transport == defs.TRANSPORT_TCP) {
            if (!settings.tcpport) {
    ...
    peernet.put = function (key, value, callback) {
        ...
        var put_via_ws = function (k, v, donecb) { 
            var protocol = doorclient.globals.wsprotocol || "https";
        ...
        var dhtpeer_proc = function (k, v) {
            ...
            if (self.validate_ws_publish()) {
                ...
                put_via_ws(k, v, function (err, response) {
                    if (err) {
                        doorclient.notify.error("WS put error %j", err, true);
                    }
                    if (!response || !response.hasOwnProperty("result") || response.result != 0) {
                        doorclient.notify.error("Invalid websrvc response or websrvc failed", null, true);
                    }
                    else {
    ...
    peernet.register_at_ws = function () {
        ...
        var resultfunc = function (err, data) {
            if (err) {
                doorclient.notify.error("WS register error %j", err, true);
            }
            else if (!data || !data.hasOwnProperty("result") || data.result != 0) {
                doorclient.notify.error("Invalid websrvc response at contact register", (data && data.error) ? data.error : null, true);
            }
            else {
    ...
    peernet.init = function () {
        ...
        return new Promise((resolve, reject) => {
            logger.debug("Initialize DoorClient Network");
            ...
            peernet.connect(settings.transport, function (err) {
                ...
                if (peernet.netclient instanceof WsNet ||
                    ...
                    return reject("Error in connecting to the DoorClient network, " + (err.message ? err.message : err));
                }
                    ...
                    if (!usews) {
                        return reject("Not connected to the DoorClient network. Error: " + (err.message ? err.message : err));
                    }
    ...
    peernet.ping_to_net = function () {
        if (!appsrvc.net_connected) {
            return alert("Unable to ping. You are not connected to the DoorClient network");
        }
        alert("ping to net");
    };
